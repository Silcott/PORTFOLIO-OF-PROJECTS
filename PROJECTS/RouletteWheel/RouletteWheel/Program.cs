...
namespace RouletteWheel
{
	...
	class Casino
	{
		...
		public static void Main(string[] args)
		{
			...
			int amountWaged = player.CheckMoneyStatus();
			AddSpace(2);
			...
			var rouletteConditional = true;
			
			//check if console readline input is a number or not
			static bool IsNumber(string input)
            {
				BigInteger temp_big_int;
				var is_number = BigInteger.TryParse(input, out temp_big_int);
				return is_number;
            }

			void consoleInputConverted()
			{
				bool isNumberValue = IsNumber(ConsoleInput());
				if (isNumberValue == true)
				{
					try
					{
						//Take the sting input from the console readline method and convert into a integer then assign it to the variable to have the same data type
						amountWaged = Convert.ToInt32(isNumberValue);
					}
					catch (Exception)
					{
			...
			while (rouletteConditional == true)
            {
				...
				else
				{
					...
					void wageSelection()
					{
						...
						if (amountWaged > 0)
						{
							//try block incase someone inputs somethiing other than a positive integer 
							try
							{
								consoleInputConverted();
								...
								else if (amountWaged < player.CheckMoneyStatus())
								{
									//Use the subtract method called from the Account class and pass the variable amountBetted using the ref keywaord, which is needed to pass an argument to a method
									//Wage minus Current Money
									player.Subtract(ref amountWaged);
							...
							//catch block to handle any exceptions such as incorrect format
							catch (Exception)
							{
								Write("Wrong input, try again.  Use a number 1, 2, 3 ...\n");
								player.Bank();
							...
							case 1:
								...
								void PickANUmber()
								{
									...
									if (pickANumber != "0" && pickANumber != "00")
									{
										try
										{
											int convertPickANUmber = int.Parse(pickANumber.ToLower());
									...
									else
									{
										if (pickANumber == "0")
										{
											Console.WriteLine($"You selected {pickANumber}");
										}
										else
										{
											Console.WriteLine($"You selected {pickANumber}");
